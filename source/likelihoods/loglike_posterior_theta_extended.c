		...
		char * pch = strtok (buffer, BLANKS );
  		while( pch != NULL ){
			...
			if( strcmp(pch,"Dtheta")==0 ){
    			pch = strtok (NULL, BLANKS );
				db.Dtheta = atoi( pch );
                db.IDtheta = (int *) malloc( db.Dtheta*sizeof(int) );
				break;
			}

            if( strcmp(pch,"IDtheta")==0 ){
                if( db.Dtheta==0 || db.IDtheta==NULL  ){
                    printf("Define 'Dtheta' before 'IDtheta' in %s. Exit...",file);
                    exit(EXIT_FAILURE);
                }
                for(int i=0; i<db.Dtheta; i++){ //XXX: more checks here
                    pch = strtok (NULL, BLANKS );
                    db.IDtheta[i] = atoi( pch );
                }
                break;
            }
...
void print_db_file( const char *file ){
	...
	printf("Number of used psi dim. : %d \n", db.Dpsi );
	printf("Number of max  psi dim. : %d \n", db.MAXpsi );

	printf("Global IDs of inferred theta: ");
    for (int i=0; i<db.Dtheta; ++i) {
        printf("%d ", db.IDtheta[i]);
    }
...
void loglike_posterior_theta_initialize( ) {
    ...
    for( int i=0; i < db.Npsi; i++) {
        double ignore;
        int itheta = 0;
        for( int j=0; j<(int)((db.MAXpsi+0.5)/2); j++ ) {
            if ( j == (db.IDtheta[itheta]-1) ) {
                int id_psi1 = 2*itheta;
                int id_psi2 = 2*itheta+1;
                fscanf(fp, "%lf", &psi[i][id_psi1]);
                fscanf(fp, "%lf", &psi[i][id_psi2]);
                itheta++;
            }
            else {
                fscanf(fp, "%lf", &ignore);
